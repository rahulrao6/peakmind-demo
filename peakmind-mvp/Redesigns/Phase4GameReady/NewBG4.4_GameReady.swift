...
struct TextViewToolbar14: UIViewRepresentable {
    @Binding var text: String
    @Binding var isTextEditorFocused: Bool
    var onTap: () -> Void

    class Coordinator: NSObject, UITextViewDelegate {
        var parent: TextViewToolbar14

        init(_ parent: TextViewToolbar14) {
            self.parent = parent
        }
...
struct P4_4_5: View {
    ...
    @State private var textInputs: [String] = Array(repeating: "", count: 2) // State to store text for the 2 text fields
    ...
    @State private var focusedFieldStates: [Bool] = Array(repeating: false, count: 2) // Track focus for each field
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 30) { // Adjusted spacing for better visibility
                    ...
                    Text("Great job! What are two things you can smell?")
                    ...
                    VStack(spacing: 12) {
                        ForEach(0..<2, id: \.self) { index in // Changed to 2 text fields
                            ZStack(alignment: .leading) {
                                ...
                                TextViewToolbar14(text: $textInputs[index], isTextEditorFocused: $focusedFieldStates[index]) {
                    ...
                    .fullScreenCover(isPresented: $navigateToNextScreen) {
                        P4_4_6(closeAction: closeAction)
                    }
                }
                .padding(.horizontal)
                .onAppear {
