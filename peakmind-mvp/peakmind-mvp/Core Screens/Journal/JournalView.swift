...
struct JournalView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $isAddingEntry) {
                AddJournalEntryView(journalEntries: $journalEntries)
                    .onDisappear {
                        // Save the journal entries to Firestore
                        Task {
            ...
            .fullScreenCover(isPresented: $showJournalPrompt) {
                JournalPromptView(question: currentQuestion, isPromptAnswered: $isPromptAnswered, journalEntries: $journalEntries)
                    .onDisappear {
                        // Save the journal entries to Firestore after answering the prompt
                        Task {
                            try await viewModel.saveJournalEntry(entry: journalEntries.last!)
                        }
                    }
            ...
            .onAppear {
                // Fetch journal entries when the view appears
                viewModel.fetchJournalEntries { entries in
                    self.journalEntries = entries
                    // Check if the last journal entry is from today
    ...
    func checkIfPromptAnswered() {
        guard let lastEntry = journalEntries.first else {
            // No entries, meaning prompt has not been answered
