...
struct VCPopup: View {
    ...
    var amount: Int
    var bonus: Int
    var onDismiss: () -> Void // Closure property for onDismiss action
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                 ...
                 Button("Exit") {
                     shown.toggle()
                     // Call the onDismiss closure when the popup is dismissed
                     onDismiss()
                 }.frame(width: UIScreen.main.bounds.width/2-30, height: 40)
                 .foregroundColor(.white)
...
struct CustomAlert_Previews: PreviewProvider {
    static var previews: some View {
        VCPopup(shown: .constant(false), storeShown: .constant(false), isSuccess: false, amount: 200, bonus: 50, onDismiss: {})
    }
}
