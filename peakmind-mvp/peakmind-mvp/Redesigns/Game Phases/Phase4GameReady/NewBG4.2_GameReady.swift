...
struct TextViewToolbar12: UIViewRepresentable {
    @Binding var text: String
    @Binding var isTextEditorFocused: Bool
    var onTap: () -> Void

    class Coordinator: NSObject, UITextViewDelegate {
        var parent: TextViewToolbar12

        init(_ parent: TextViewToolbar12) {
            self.parent = parent
        }
...
struct P4_4_3: View {
    ...
    @State private var focusedFieldStates: [Bool] = Array(repeating: false, count: 4) // Track focus for each field
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 30) { // Adjusted spacing for better visibility
                    ...
                    // Non-scrollable Text Fields using TextViewToolbar
                    VStack(spacing: 12) {
                        ForEach(0..<4, id: \.self) { index in
                            ZStack(alignment: .leading) {
                                ...
                                TextViewToolbar12(text: $textInputs[index], isTextEditorFocused: $focusedFieldStates[index]) {
